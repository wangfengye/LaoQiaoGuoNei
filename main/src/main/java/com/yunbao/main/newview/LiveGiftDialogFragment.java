...
public class LiveGiftDialogFragment extends AbsDialogFragment implements View.OnClickListener, OnItemClickListener<String>, LiveGiftPagerAdapter.ActionListener {
    ...
    public void setLiveGuardInfo(LiveGuardInfo liveGuardInfo,LiveAudienceViewHolder holder) {
        mLiveGuardInfo = liveGuardInfo;this.mHolder =holder;
    }
            ...
            if (code == 0) {
                if (info.length > 0) {
                    ...
                    if (mContext != null && mGiftBean != null) {
                        mHolder.onCoinChanged(coin);
                        mHolder.sendGiftMessage(mGiftBean, obj.getString("gifttoken"));
                    }
