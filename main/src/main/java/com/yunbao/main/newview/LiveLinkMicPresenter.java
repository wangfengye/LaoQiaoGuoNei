...
public class LiveLinkMicPresenter implements View.OnClickListener {
    ...
    public LiveLinkMicPresenter(Context context, ILiveLinkMicViewHolder linkMicViewHolder, boolean isAnchor, int liveSdk, View root,LiveViewHolder holder) {
        mContext = context;
        mHolder=holder;
        mRoot = root;
        mIsAnchor = isAnchor;
        ...
        if (!isAnchor && root != null) {
            View btnLinkMic = root.findViewById(com.yunbao.live.R.id.btn_link_mic);
            btnLinkMic.setVisibility(View.VISIBLE);
            btnLinkMic.setOnClickListener(this);
            mLinkMicTip = btnLinkMic.findViewById(com.yunbao.live.R.id.link_mic_tip);
        }
        mLinkMicWaitString = WordUtil.getString(com.yunbao.live.R.string.link_mic_wait);
    ...
    public void onAudienceApplyLinkMic(UserBean u) {
        ...
        if (isLinkMic() || (mHolder).isLinkMicAnchor()||(mHolder).isGamePlaying()) {
    ...
    public void onAnchorAcceptLinkMic() {
        if (!mIsAnchor) {
            ...
            ToastUtil.show(com.yunbao.live.R.string.link_mic_anchor_accept);
            mIsLinkMic = true;
            mLinkMicUid = CommonAppConfig.getInstance().getUid();
            if (mLinkMicTip != null) {
                mLinkMicTip.setText(com.yunbao.live.R.string.live_link_mic_3);
            }
                    ...
                    if (code == 0 && info.length > 0) {
                        ...
                        mLiveLinkMicPushViewHolder.setLivePushListener(new LivePushListener() {
                            ...
                            public void onPushFailed() {//推流失败的回调
                                DialogUitl.showSimpleDialog(mContext, WordUtil.getString(com.yunbao.live.R.string.link_mic_failed_2), null);
    ...
    public void onAnchorRefuseLinkMic() {
        mLastApplyLinkMicTime = 0;
        ToastUtil.show(com.yunbao.live.R.string.link_mic_refuse);
    }
    ...
    private void closeLinkMic(String uid, String uname) {
        if (!TextUtils.isEmpty(uid) && uid.equals(mLinkMicUid)) {
            ToastUtil.show(uname + WordUtil.getString(com.yunbao.live.R.string.link_mic_exit));
            if (!mIsAnchor && !TextUtils.isEmpty(mLinkMicUid) && mLinkMicUid.equals(CommonAppConfig.getInstance().getUid())) {//参与连麦的是自己
                ...
                if (mLinkMicTip != null) {
                    mLinkMicTip.setText(com.yunbao.live.R.string.live_link_mic_2);
                }
                if (mLiveSdk == Constants.LIVE_SDK_TX) {
    ...
    public void onAnchorNotResponse() {
        mLastApplyLinkMicTime = 0;
        ToastUtil.show(com.yunbao.live.R.string.link_mic_anchor_not_response);
    }
    ...
    public void onAnchorBusy() {
        mLastApplyLinkMicTime = 0;
        ToastUtil.show(com.yunbao.live.R.string.link_mic_anchor_busy);
    }
    ...
    public void onClick(View v) {
        ...
        int i = v.getId();
        if (i == com.yunbao.live.R.id.btn_link_mic) {
            onLinkMicBtnClick();

        } else if (i == com.yunbao.live.R.id.btn_refuse) {
            anchorRefuseLinkMicApply();

        } else if (i == com.yunbao.live.R.id.btn_accept) {
            anchorAcceptLinkMicApply();

        } else if (i == com.yunbao.live.R.id.btn_close_link_mic) {
            anchorCloseLinkMic();
    ...
    private void onLinkMicBtnClick() {
        if (mHolder.isGamePlaying()) {
            ToastUtil.show(com.yunbao.live.R.string.live_game_cannot_link_mic);
            return;
        }
        if ((mHolder).isLinkMicAnchor()) {
            ToastUtil.show(com.yunbao.live.R.string.live_link_mic_cannot_link);
            return;
        }
        if (mIsLinkMic) {
            SocketLinkMicUtil.audienceCloseLinkMic(mSocketClient);
    ...
    private void audienceApplyLinkMic() {
        ...
        if (curTime - mLastApplyLinkMicTime < 11000) {//时间间隔11秒
            ToastUtil.show(com.yunbao.live.R.string.link_mic_apply_waiting);
        } else {
            mLastApplyLinkMicTime = curTime;
            //请求权限
            ProcessResultUtil processResultUtil = (mHolder).getProcessImageUtil();
            ...
            processResultUtil.requestPermissions(new String[]{
                ...
                public void run() {
                    ...
                    ToastUtil.show(com.yunbao.live.R.string.link_mic_apply);
                }
            });
        }
    }
    ...
    private void showLinkMicDialog(UserBean u) {
        ...
        View v = LayoutInflater.from(mContext).inflate(com.yunbao.live.R.layout.dialog_link_mic_wait, null);
        ImageView avatar = (ImageView) v.findViewById(com.yunbao.live.R.id.avatar);
        TextView name = (TextView) v.findViewById(com.yunbao.live.R.id.name);
        ImageView sex = (ImageView) v.findViewById(com.yunbao.live.R.id.sex);
        ImageView level = (ImageView) v.findViewById(com.yunbao.live.R.id.level);
        mLinkMicWaitText = v.findViewById(com.yunbao.live.R.id.wait_text);
        v.findViewById(com.yunbao.live.R.id.btn_refuse).setOnClickListener(this);
        v.findViewById(com.yunbao.live.R.id.btn_accept).setOnClickListener(this);
        ...
        mLinkMicPopWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
            ...
            public void onDismiss() {
                ...
                if (mAcceptLinkMic) {
                    ...
                    if (mHolder.isLinkMicAnchor()) {
                        ToastUtil.show(com.yunbao.live.R.string.live_link_mic_cannot_link_2);
                        return;
                    }
    ...
    private void anchorAcceptLinkMicApply() {
        //un finish aurhor
        /*
        if (mHolder.isBgmPlaying()) {
            DialogUitl.showSimpleDialog(mContext, WordUtil.getString(com.yunbao.live.R.string.link_mic_close_bgm), new DialogUitl.SimpleCallback() {
                @Override
