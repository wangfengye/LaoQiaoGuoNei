...
public class LiveRoomViewHolder  extends AbsViewHolder implements View.OnClickListener {
    ...
    private LiveRoomViewHolder.LiveRoomHandler mLiveRoomHandler;
    ...
    public LiveRoomViewHolder(Context context, ViewGroup parentView, GifImageView gifImageView, SVGAImageView svgaImageView,LiveAudienceViewHolder holder) {
        super(context, parentView,gifImageView,svgaImageView,holder);
    ...
    protected void processArguments(Object... args) {
        ...
        mHolder= (LiveAudienceViewHolder) args[2];
    }

    @Override
    protected int getLayoutId() {
        return com.yunbao.live.R.layout.view_live_room;
    }

    @Override
    public void init() {
        mRoot = (ViewGroup) findViewById(com.yunbao.live.R.id.root);
        mAvatar = (ImageView) findViewById(com.yunbao.live.R.id.avatar);
        mLevelAnchor = (ImageView) findViewById(com.yunbao.live.R.id.level_anchor);
        mName = (TextView) findViewById(com.yunbao.live.R.id.name);
        mID = (TextView) findViewById(com.yunbao.live.R.id.id_val);
        mBtnFollow = findViewById(com.yunbao.live.R.id.btn_follow);
        mVotesName = (TextView) findViewById(com.yunbao.live.R.id.votes_name);
        mVotes = (TextView) findViewById(com.yunbao.live.R.id.votes);
        mGuardNum = (TextView) findViewById(com.yunbao.live.R.id.guard_num);

        rl_shop_icon= (ViewGroup) findViewById(com.yunbao.live.R.id.rl_shop_icon);
        img_shop= (ImageView) findViewById(com.yunbao.live.R.id.img_shop);

        //用户头像列表
        mUserRecyclerView = (RecyclerView) findViewById(com.yunbao.live.R.id.user_recyclerView);
        ...
        //聊天栏
        mChatRecyclerView = (RecyclerView) findViewById(com.yunbao.live.R.id.chat_recyclerView);
        ...
        findViewById(com.yunbao.live.R.id.btn_votes).setOnClickListener(this);
        findViewById(com.yunbao.live.R.id.btn_guard).setOnClickListener(this);
        mBtnRedPack = findViewById(com.yunbao.live.R.id.btn_red_pack);
        mBtnRedPack.setOnClickListener(this);
        if (mContext instanceof MainActivity) {
            mRoot.setOnClickListener(this);
        } else {
            mLiveTimeTextView = (TextView) findViewById(com.yunbao.live.R.id.live_time);
        ...
        mLiveRoomHandler = new LiveRoomViewHolder.LiveRoomHandler(this);
        ...
        mTimeChargeCallback = new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (mHolder instanceof LiveAudienceViewHolder) {
                    final LiveAudienceViewHolder liveAudienceActivity = (LiveAudienceViewHolder) mHolder;
                    if (code == 0) {
                        ...
                        if (code == 1008) {//余额不足
                            ...
                            DialogUitl.showSimpleDialog(mContext, WordUtil.getString(com.yunbao.live.R.string.live_coin_not_enough), false,
        ...
        btn_coupon=findViewById(com.yunbao.live.R.id.btn_coupon);
        btn_coupon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((LiveViewHolder)mHolder).openCouponDialog();
            }
        });
    }
    ...
    public void setGuardNum(int guardNum) {
        if (mGuardNum != null) {
            if (guardNum > 0) {
                mGuardNum.setText(guardNum + WordUtil.getString(com.yunbao.live.R.string.ren));
            } else {
                mGuardNum.setText(com.yunbao.live.R.string.main_list_no_data);
            }
        }
    }
    ...
    public void onClick(View v) {
        ...
        int i = v.getId();
        if (i == com.yunbao.live.R.id.avatar) {
            showAnchorUserDialog();

        } else if (i == com.yunbao.live.R.id.btn_follow) {
            follow();

        } else if (i == com.yunbao.live.R.id.btn_votes) {
            openContributeWindow();

        } else if (i == com.yunbao.live.R.id.btn_guard) {
            (mHolder).openGuardListWindow();

        } else if (i == com.yunbao.live.R.id.root) {
            light();

        } else if (i == com.yunbao.live.R.id.btn_red_pack) {
            (mHolder).openRedPackListWindow();
    ...
    private void follow() {
        ...
        CommonHttpUtil.setAttention(mLiveUid, new CommonCallback<Integer>() {
            ...
            public void callback(Integer isAttention) {
                if (isAttention == 1) {
                    mHolder.sendSystemMessage(
                            CommonAppConfig.getInstance().getUserBean().getUserNiceName() + WordUtil.getString(com.yunbao.live.R.string.live_follow_anchor));
                }
            }
        });
    }
    ...
    private void showUserDialog(String toUid) {
        if (!TextUtils.isEmpty(mLiveUid) && !TextUtils.isEmpty(toUid)) {
            ...
            fragment.show(((AppCompatActivity)mContext).getSupportFragmentManager(), "LiveUserDialogFragment");
        }
    }
    ...
    public ViewGroup getInnerContainer() {
        return (ViewGroup) findViewById(com.yunbao.live.R.id.inner_container);
    }
